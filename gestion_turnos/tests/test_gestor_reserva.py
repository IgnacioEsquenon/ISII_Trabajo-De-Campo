...
class TestGestorReservaReservarTurno(TestCase):
    ...
    def test_CP09_reserva_exitosa(self):
        """CP-09: Turno disponible y paciente válido crea la reserva."""
    ...
    def test_CP10_turno_ya_reservado_lanza_error(self):
        """CP-10: Intentar reservar un turno ya reservado lanza ValidationError."""
        self.turno.bloquear()
        with self.assertRaises(ValidationError):
            self.gestor.reservar_turno(self.paciente, self.turno)

    def test_CP11_codigo_reserva_generado(self):
        """CP-11: La reserva tiene un código único que empieza con RES-."""
        reserva = self.gestor.reservar_turno(self.paciente, self.turno)
        self.assertTrue(reserva.codigo_reserva.startswith('RES-'))

    def test_CP12_turno_pasa_a_reservado(self):
        """CP-12: Después de reservar el turno queda bloqueado."""
        self.gestor.reservar_turno(self.paciente, self.turno)
        self.turno.refresh_from_db()
        self.assertTrue(self.turno.esta_reservado)

    def test_CP13_turno_inactivo_no_reservable(self):
        """CP-13: Turno inactivo no puede reservarse."""
...
class TestGestorReservaCancelar(TestCase):
    ...
    def test_CP14_cancelar_reserva_exitosa(self):
        """CP-14: Cancelar reserva con más de 24hs cambia estado a cancelada."""
    ...
    def test_CP17_cancelar_reserva_libera_turno(self):
        """CP-17: Cancelar reserva libera el turno."""
        self.gestor.cancelar_reserva(self.reserva)
        self.turno.refresh_from_db()
        self.assertFalse(self.turno.esta_reservado)

    def test_CP18_estado_pasa_a_cancelada(self):
        """CP-18: Al cancelar la reserva el estado queda en cancelada."""
        self.gestor.cancelar_reserva(self.reserva)
        self.reserva.refresh_from_db()
        self.assertEqual(self.reserva.estado, 'cancelada')